...
const TARGET_HOST = process.env.TARGET_HOST || '1.1.1.1';
...
async function checkSocks5(proxy, timeout) {
...
async function checkSocks4(proxy, timeout) {
  const socket = await connectSocket(proxy.host, proxy.port, timeout);
  try {
    const user = Buffer.from(proxy.username || '');
    const hostParts = TARGET_HOST.split('.').map(Number);
    const req = Buffer.concat([
      Buffer.from([0x04, 0x01, (TARGET_PORT >> 8) & 0xff, TARGET_PORT & 0xff, ...hostParts]),
      user,
      Buffer.from([0x00])
    ]);
    socket.write(req);
...
async function checkProxy(proxy, timeout) {
  ...
  try {
    if (proxy.type === 'socks4') {
      await checkSocks4(proxy, timeout);
    } else {
      await checkSocks5(proxy, timeout);
    }
    return {
      proxy: proxy.raw,
      type: proxy.type,
      ...
      note: `Connected to ${TARGET_HOST}:${TARGET_PORT}`
    };
  } catch (err) {
    return {
      proxy: proxy.raw,
...
async function runJob(job) {
  ...
  async function worker() {
    while (true) {
      ...
      const result = await checkProxy(proxy, job.timeout);
      done += 1;
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/jobs/start') {
    try {
      ...
      const job = {
        ...
        proxies,
        results: [],
        logs: [{ time: new Date().toISOString().slice(11, 19), level: 'info', message: 'Job accepted by engine.' }],
        summary: {
  ...
  if (req.method === 'GET' && req.url === '/health') {
    return json(res, 200, { ok: true, service: 'proxy-engine-node' });
  }
